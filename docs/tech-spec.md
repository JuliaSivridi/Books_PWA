...
12. [PWA & Offline Mode](#12-pwa--offline-mode)
...
| PWA / Service Worker | vite-plugin-pwa | ^1.x | Generates Workbox SW that precaches app shell (HTML/JS/CSS/SVG) for offline use |
...
initAuth(clientId)
  ...
  │    └─ yes → notify(true) → DONE (no GIS call needed)
  │
  ├─ Offline path: !navigator.onLine && getUser()?
  │    └─ yes → notify(true) → DONE (cached books available, no token needed)
  │
...
initAuth(CLIENT_ID) called
  │
  ├─ isAuth = true →
  │    ├─ Offline shortcut: !navigator.onLine && books_sheet_id in localStorage
  │    │    └─ setPhase('ready')   ← skips Drive API call
  │    │
  │    └─ checkBooksFile()
  │         ├─ 'ready' → setPhase('ready')
  │         ├─ 'setup' → setPhase('setup')   ← SetupScreen shown
  │         └─ throws (network error) →
  │              ├─ offline + books_sheet_id → setPhase('ready')
  │              └─ else → setDriveError(msg); setPhase('login')
  │
...
**Offline behaviour:** if `navigator.onLine` is false, `load()` returns immediately after dispatching the cached data — `initializeSheet()` and `fetchBooks()` are not called. Books/authors/series views work fully from the IndexedDB snapshot; covers are served from the browser's HTTP cache. Writing (add/edit) is not available offline.
...
Generated by `vite-plugin-pwa` (Workbox `generateSW` mode). Configured in `vite.config.ts`:
...
VitePWA({
  ...
  workbox: {
    ...
    runtimeCaching: [],   // fonts and covers served from browser HTTP cache
  },
  manifest: false,   // keep existing public/manifest.json
})
```

On first online visit the SW installs and **precaches the full app shell** (HTML, JS, CSS, SVG icons, ~220 KB gzipped). Subsequent visits — including offline ones — are served entirely from the SW cache.

### Offline flow

1. SW serves the app shell from precache → app boots.
2. `initAuth`: if `navigator.onLine === false` and a user profile is in `localStorage`, calls `notify(true)` without waiting for the GIS script (which is on `accounts.google.com` and unreachable offline).
3. `AppInner`: if offline and `books_sheet_id` is in `localStorage`, skips `checkBooksFile()` (Drive API) and sets phase to `'ready'` directly.
4. `BooksContext.load()`: if offline, dispatches the IndexedDB-cached book list and returns — no Sheets API calls are made.

**What works offline:** browse books, filter by status/type/genre, sort by title/author/series, search. Cover images are served from the browser's own HTTP cache.

**What does not work offline:** adding or editing books (requires Sheets API write), first-time login (requires GIS), setup screen (requires Drive API).
